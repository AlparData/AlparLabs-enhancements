...
class AcruxChatConversation(models.Model):
    ...
    def new_message_hook(self, message_id, limit, data):
        limit, send_bus = super(AcruxChatConversation, self).new_message_hook(message_id, limit, data)
        _logger.info(f"SIT conversation.BOT-----------> new_message_hook message_id={message_id}")
        _logger.info("SIT SIT  new_message_hook limit=%s, sendbus=%s",limit,send_bus)
        _logger.info("SIT SIT  new_message_hook message_id.contact_id.status=%s",message_id.contact_id.status)
        if message_id.contact_id.status != 'current':
            rollback = False
            try:
                messages = self.env['acrux.chat.bot']._bot_get(message_id)
                _logger.info("SIT SIT ---------->  new_message_hook bot messages=%s",messages)
                if messages:
