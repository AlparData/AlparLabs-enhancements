...
this.render(true)}})
useBus(this.env.chatBus,'updateConversation',async()=>{if(!this.isButtonActive){await this.model.load()}})}},async openRecord(record,mode){if(this.props?.chatroomTab&&this.props?.chatroomOpenRecord){await this.props?.chatroomOpenRecord(record,mode)}else{await super.openRecord(record,mode)}},async beforeExecuteActionButton(clickParams){return super.beforeExecuteActionButton(clickParams).then(res=>{this.isButtonActive=res
return res})},async afterExecuteActionButton(clickParams){return super.beforeExecuteActionButton(clickParams).then(res=>{this.isButtonActive=false
return res})},}
