...
export class ChatroomActionTab extends Component {
    ...
    async willStart() {
        await this.makeAction(this.props);
    }

    async onWillUpdateProps(nextProps) {
        await this.makeAction(nextProps);
    }
    ...
    onActionManagerUpdate({ detail: info }) {
        if (info?.componentProps?.chatroomTab === this.props.viewKey) {
            ...
            this.info.Component = class ChatroomController extends info.Component {
                onMounted() {
                    const origin = JSON.parse(JSON.stringify(router.current));
    ...
    getActionConfig(props) {
        ...
        return {
            ...
            view_type: 'form',
            view_mode: props.viewType,
            res_model: props.viewModel,
            views: this.getActionViews(props),
            target: 'current',
            context: context,
            res_id: props.viewResId,
            flags: this.getActionFlags(props),
            name: props.viewTitle,
        };
    }

    getActionViews(props) {
        let out;
        if (props.viewType === 'form') {
            out = [[props.viewId, props.viewType]];
        } else if (props.viewType === 'list' || props.viewType === 'kanban') {
            out = [[props.viewId, props.viewType], [false, 'search']];
        } else {
    ...
    _contextHook(context) {
        if ((['res.partner', 'crm.lead'].includes(this.props.viewModel) &&
            this.props.selectedConversation?.isOwnerFacebook() &&
            !this.props.selectedConversation?.isWabaExtern()) ||
            this.props.selectedConversation?.isPrivate ||
            this.props.selectedConversation?.isGroup ||
            this.props.selectedConversation?.isWechat()) {
    ...
    getExtraContext(props) {
        return { default_conversation_id: props?.selectedConversation?.id };
    }

    getActionFlags(props) {
        let out;
        if (props.viewType === 'form') {
            out = { withControlPanel: false, footerToButtons: false, hasSearchView: false, hasSidebar: false, mode: 'edit', searchMenuTypes: false };
        } else if (props.viewType === 'list' || props.viewType === 'kanban') {
            out = { withControlPanel: true, footerToButtons: false, hasSearchView: true, hasSidebar: false, searchMenuTypes: ['filter', 'groupBy'], withSearchPanel: true, withSearchBar: true };
        } else {
            throw new Error('Not implemented.');
        }
        return out;
    ...
    getActionOptions(props) {
        ...
        if (this.env.chatroomJsId === this.env.services.action.currentController.action.jsId) {
            stackPosition = false;
        }
    ...
    getActionProps(props) {
        ...
        if (props.viewType === 'form') {
            Object.assign(out, {
                ...
                searchButtonString: props.searchButtonString || _t('Search'),
                searchAction: this._onSearchChatroom.bind(this)
            });
        }
        return out;
    }
    ...
    _getSearchAction() {
        ...
        return {
            ...
            view_type: 'form',
            view_mode: 'list',
            res_model: this.props.viewModel,
    ...
    async onSave(record) {
        if (this.props.viewType === 'form') {
            if (record.data.partner_id) {
                if (record.data.partner_id[0] !== this.props.selectedConversation.partner.id) {
                    await this.savePartner(record.data.partner_id);
                }
            }
        }
    }
...
ChatroomActionTab.defaultProps = {};
